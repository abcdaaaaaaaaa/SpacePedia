...
if(empty(trim($_POST['theme']??''))){goHome();}
...
if($_SERVER["REQUEST_METHOD"]=="POST"){
    ...
    if(isset($_FILES['cover']) && $_FILES['cover']['error']===UPLOAD_ERR_OK){
        ...
        move_uploaded_file($_FILES['cover']['tmp_name'],'blog/'.$cover_name);
    }

    $table = ($lang === 'en') ? 'blog_posts2' : 'blog_posts';

    $stmt = $db->prepare("INSERT INTO {$table} (user_id,title,subject,theme,summary,content,cover,visibility) VALUES (:user_id,:title,:subject,:theme,:summary,:content,:cover,:visibility)");
    ...
    if($lang === 'en'){
        if($visibility == 1){ header("Location: /tutorial_articles"); }
        else { header("Location: /@/$username/tutorial_articles"); }
    } else {
        if($visibility == 1){ header("Location: /ogreticimakaleler"); }
        else { header("Location: /@/$username/ogreticimakaleler"); }
    }
    exit();
}
?>
