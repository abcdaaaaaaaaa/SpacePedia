...
<title><?php echo htmlspecialchars($course['title']); ?> -  Bölüm Videolar</title>
...
<body>
<div class="container">
    <h1><?php echo htmlspecialchars($course['title']); ?> - Bölüm Videoları</h1>

    <?php if (!empty($videos)): ?>
        <?php foreach ($videos as $index => $video): ?>
            <div class="video-box <?php echo $index === 0 ? 'active' : ''; ?>" data-index="<?php echo $index; ?>">
                <h3>Bölüm <?php echo $index+1; ?></h3>
                <?php
                $youtube_id = '';
        ...
        <div class="controls">
            <button id="prevBtn" class="hidden">← Önceki</button>
            <button id="nextBtn">Sonraki →</button>
        </div>

    <?php else: ?>
        <p>Bu kurs için bölüm videoları bulunmamaktadır.</p>
    <?php endif; ?>
</div>
