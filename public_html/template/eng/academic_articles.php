...
<title>All Academic Articles</title>
...
try {
    $params = [];
    $sql = "SELECT article_posts2.*, users.username 
            FROM article_posts2 
            LEFT JOIN users ON article_posts2.user_id = users.id 
            WHERE article_posts2.visibility = 1";
    if ($search) {
        $sql .= " AND (
            article_posts2.title LIKE :search 
            OR article_posts2.subject LIKE :search 
            OR article_posts2.summary LIKE :search
            OR article_posts2.purpose LIKE :search
            OR article_posts2.audience LIKE :search
            OR users.username LIKE :search
            OR DATE_FORMAT(article_posts2.created_at, '%d-%m-%Y') LIKE :search
        )";
    ...
    if ($articles) {
        foreach ($articles as $article) {
            $url = !empty($article['pdf']) ? 'https://www.spacepedia.info/article/'.htmlspecialchars($article['pdf']) : '#';
            echo '<a class="article-box" href="'.$url.'" target="_blank">';
            if (!empty($article['cover'])) echo '<img src="https://www.spacepedia.info/article/'.htmlspecialchars($article['cover']).'" alt="'.htmlspecialchars($article['title']).'">';
            echo '<div class="article-info">';
            echo '<h2>'.htmlspecialchars($article['title']).'</h2>';
            echo '<p><b>Author:</b> '.htmlspecialchars($article['username']).'</p>';
            echo '<p><b>Subject:</b> '.htmlspecialchars($article['subject']).'</p>';
            echo '<p><b>Summary:</b> '.htmlspecialchars($article['summary']).'</p>';
            echo '<p><b>Date:</b> '.htmlspecialchars(date("d-m-Y", strtotime($article['created_at']))).'</p>';
            echo '</div></a>';
        }
    } else {
        echo "<div class='no-articles'>No results matched your search for academic articles.</div>";
    }
