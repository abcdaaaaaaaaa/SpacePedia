...
<title>All Tutorial Articles</title>
...
try {
    ...
    $sql = "SELECT blog_posts2.*, users.username 
            ...
            WHERE blog_posts2.visibility = 1";
    if ($search) {
        $sql .= " AND (
            ...
            OR DATE_FORMAT(blog_posts2.created_at, '%d-%m-%Y') LIKE :search
        )";
    ...
    if ($articles) {
        foreach ($articles as $article) {
            ...
            echo '<a class="article-box" target="_blank" href="/tutorial_article_detail?id='.$article['id'].'">';
