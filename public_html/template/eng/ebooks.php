...
<form method="get" action="">
    <input type="text" name="search" placeholder="Search for Ebooks..." value="<?php echo htmlspecialchars($search); ?>">
...
try {
    $params = [];
    $sql = "SELECT ebook_posts2.*, users.username 
            FROM ebook_posts2 
            LEFT JOIN users ON ebook_posts2.user_id = users.id 
            WHERE ebook_posts2.visibility = 1";
    if ($search) {
        $sql .= " AND (
            ebook_posts2.title LIKE :search 
            OR ebook_posts2.summary LIKE :search
            OR users.username LIKE :search
            OR DATE_FORMAT(ebook_posts2.created_at, '%d-%m-%Y') LIKE :search
        )";
    ...
    if ($articles) {
        foreach ($articles as $article) {
            $url = !empty($article['pdf']) ? '/ebook_viewer?file='.urlencode($article['pdf']) : '#';
            echo '<a class="article-box" href="'.$url.'" target="_blank">';
            if (!empty($article['cover'])) echo '<img src="https://www.spacepedia.info/ebook/'.htmlspecialchars($article['cover']).'" alt="'.htmlspecialchars($article['title']).'">';
        ...
        echo "<div class='no-articles'>No results matched your search for ebooks.</div>";
    }
