...
<form method="get" action="">
    <input type="text" name="search" placeholder="Search for Simulation..." value="<?php echo htmlspecialchars($search); ?>">
...
try {
    $params = [];
    $sql = "SELECT simulation_posts2.*, users.username 
            FROM simulation_posts2 
            LEFT JOIN users ON simulation_posts2.user_id = users.id 
            WHERE simulation_posts2.visibility = 1";
    if ($search) {
        $sql .= " AND (
            simulation_posts2.title LIKE :search 
            OR simulation_posts2.features LIKE :search 
            OR simulation_posts2.description LIKE :search
            OR users.username LIKE :search
            OR DATE_FORMAT(simulation_posts2.created_at, '%d-%m-%Y') LIKE :search
        )";
    ...
    if ($sims) {
        foreach ($sims as $sim) {
            $detailLink = '/simulation_detail?id='.$sim['id'];
            ...
            if(!empty($sim['html_code'])){
                ...
                echo '<div style="text-align:center; line-height:150px; color:#999;">No preview</div>';
            }
            echo '</div>';
            echo '<div class="sim-info">';
            echo '<h2>'.htmlspecialchars($sim['title']).'</h2>';
            echo '<p><b>Author:</b> '.htmlspecialchars($sim['username']).'</p>';
            echo '<p><b>Features:</b> '.htmlspecialchars($sim['features']).'</p>';
            echo '<p><b>Descriptions:</b> '.htmlspecialchars($sim['description']).'</p>';
            echo '<p><b>Date:</b> '.htmlspecialchars(date("d-m-Y", strtotime($sim['created_at']))).'</p>';
            echo '</div></a>';
        }
    } else {
        echo "<div class='no-sims'>No results matched your search for simulations.</div>";
    }
} catch (PDOException $e) {
    echo "<div class='no-sims'>Database Error: ".$e->getMessage()."</div>";
}
?>
</div>
</body>
