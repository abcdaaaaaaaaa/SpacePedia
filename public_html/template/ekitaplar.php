...
<h1>Tüm E-Kitaplar</h1>
<form method="get" action="">
    <input type="text" name="search" placeholder="E-Kitap ara..." value="<?php echo htmlspecialchars($search); ?>">
...
try {
    $params = [];
    $sql = "SELECT ebook_posts.*, users.username 
            FROM ebook_posts 
            LEFT JOIN users ON ebook_posts.user_id = users.id 
            WHERE ebook_posts.visibility = 1";
    if ($search) {
        $sql .= " AND (
            ebook_posts.title LIKE :search 
            OR ebook_posts.summary LIKE :search
            OR users.username LIKE :search
            OR DATE_FORMAT(ebook_posts.created_at, '%d-%m-%Y') LIKE :search
        )";
    ...
    if ($articles) {
        foreach ($articles as $article) {
            $url = !empty($article['pdf']) ? '/ekitapviewer?file='.urlencode($article['pdf']) : '#';
            echo '<a class="article-box" href="'.$url.'" target="_blank">';
            if (!empty($article['cover'])) echo '<img src="https://www.uzay.info/ebook/'.htmlspecialchars($article['cover']).'" alt="'.htmlspecialchars($article['title']).'">';
        ...
        echo "<div class='no-articles'>Arama kriterine uygun e-kitap bulunamadı.</div>";
    }
